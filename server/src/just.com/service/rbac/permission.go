...
var (
	...
	ROLE_ADD_PERMISSION_ERR = errors.New(service.SERVICE_RBAC_PERMISSION_ADD_ERR)
...
func NewRoleService(session *xorm.Session, log *log.Logger) *RoleService {
	return &RoleService{
		Session:session,
		Log:log,
	}
...
func (self *RoleService) AddPermission(roleName, pTarget, pAction string) error {
	...
	if roleErr != nil {
		...
		return ROLE_ADD_PERMISSION_ERR
	}
	// get permission
	p := new(table.PermissionTable)
	p.Target = pTarget
